...
OUTPUT1 = np.mean(hiddens_output1, axis=1).reshape(-1, N_HID)
OUTPUT2 = np.mean(hiddens_output2, axis=1).reshape(-1, N_HID)
OUTPUT3 = np.mean(hiddens_output3, axis=1).reshape(-1, N_HID)
...
def process_single_model_pca(params):
    ...
    all_variance : list
        包含三个时期方差解释率的列表
    """
    hiddens_list = [hiddens_output1, hiddens_output2, hiddens_output3]
...
def plot_pca_grid_multi_models(
    ...
    row_labels,
    fig_dir,
    fig_name="output_pca_multi_models",
    ...
    params_list : list
        包含多个模型参数的列表
    color_matrix : list of list
        3x3颜色矩阵，color_matrix[row][col]用于绘制第row行第col列的散点图
    ...
    title_matrix : list of list, optional
        3x3标题矩阵，title_matrix[row][col]用于第row行第col列散点图的标题，
        如果为None，则使用默认标题"Rank-{col + 1} subspace"
    """
    ...
    for row in range(3):
        ...
        for col in range(3):
            ...
            ax.set_xticks(
                [-abs_max, 0, abs_max], labels=[f"{-abs_max}", "", f"{abs_max}"]
            )
            ax.set_ylim(-abs_max, abs_max)
            ax.set_yticks([-abs_max, 0, abs_max], labels=["", "", f"{abs_max}"])
        ...
        #     ax_var.tick_params(
        #         axis="both", which="major", width=1.5, direction="in", length=4
        #     )
...
if __name__ == "__main__":
    ...
    # 构建3x3颜色矩阵：每个[row][col]对应第row行第col列散点图的颜色
    color_matrix = [
        [colors_rank1, colors_rank2, colors_rank3],  # 第一行：Output1的3个子空间
        [colors_rank2, colors_rank3, colors_rank1],  # 第二行：Output2的3个子空间
        [colors_rank3, colors_rank2, colors_rank1],  # 第三行：Output3的3个子空间
    ]
    # 构建3x3标题矩阵：每个[row][col]对应第row行第col列散点图的标题
    title_matrix = [
        ["Rank-1 subspace", "Rank-2 subspace", "Rank-3 subspace"],  # 第一行
        ["Rank-2 subspace", "Rank-3 subspace", "Rank-1 subspace"],  # 第二行
        ["Rank-3 subspace", "Rank-2 subspace", "Rank-1 subspace"],  # 第三行
    ]
    Low_mean, variance_mean, variance_std = plot_pca_grid_multi_models(
        ...
        FIG_DIR,
        fig_name="sfig_output_pca",
        enable_error_bars=False,  # 不显示误差条
        save_variance_subplots=False,  # 单独保存方差解释率子图
        title_matrix=title_matrix,  # 可选：手动指定标题矩阵
    )
    print("所有图表绘制完成！")
