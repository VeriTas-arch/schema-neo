...
def get_data(stim_dur=4, num_train=20000, stim_123=None, class_index=None):
    stim_len = 200  # 在stim_3后面留一个dur
    train_x = np.zeros((num_train, stim_len, 27))
    train_y = np.zeros(num_train, np.int64)
    train_t = np.zeros((num_train, 6), np.int64)
    for i in range(len(train_y)):
        ...
        if class_index is None:
            class_i = np.random.choice(240)  # 实际范围是0~119
        else:
        ...
        train_x[i, 0 : stim3 + stim_dur + stim4 + 4 + 10 + stim5, -1] = 1

        train_x[
            i, stim3 + stim_dur + stim4 + 4 : stim3 + stim_dur + stim4 + 4 + 10, 24:25
        ] = stim_value_4
        train_x[
            i, stim3 + stim_dur + stim4 + 4 : stim3 + stim_dur + stim4 + 4 + 10, 25:26
        ] = stim_value_5
        ...
        train_t[i, 4] = int(stim5)
        train_t[i, 5] = int(stim_len)
...
def get_regress_target_long(class_i, train_t, stim_len):
    ...
    if class_i >= 120:
        ...
        p.remove(item_2)
        item_3 = p[cue3]
        reg_out[t3 + t4 + 8 + t5 + 10 : t3 + t4 + 8 + t5 + 10 + 12, item_3] = 1
        reg_out[
            t3 + t4 + 8 + t5 + 10 + 12 : t3 + t4 + 8 + t5 + 10 + 12 + 12, item_2
        ] = 1
        reg_out[
            t3 + t4 + 8 + t5 + 10 + 12 + 12 : t3 + t4 + 8 + t5 + 10 + 12 + 12 + 12,
            item_1,
        ] = 1
        class_i = class_i + 120
    else:
        ...
        p.remove(item_2)
        item_3 = p[cue3]
        reg_out[t3 + t4 + 8 + t5 + 10 : t3 + t4 + 8 + t5 + 10 + 12, item_1] = 1
        reg_out[
            t3 + t4 + 8 + t5 + 10 + 12 : t3 + t4 + 8 + t5 + 10 + 12 + 12, item_2
        ] = 1
        reg_out[
...
class switchDataset(Dataset):
    ...
    def get_timepoints(self):
        ...
        pre_cue_start = stim3_end
        cue_start = pre_cue_start + data_t_i[3]
        cue_end = cue_start + cue_len
        post_cue_start = cue_end
        post_cue_end = post_cue_start + data_t_i[4]

        delay_start = stim3_end
        delay_end = post_cue_end

        target1_start = post_cue_end
